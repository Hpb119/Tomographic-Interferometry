...
N = 1000;           % The discretization points.
theta = 0:5:179;  % No. of used angles.
p = 1000;           % No. of parallel rays.
...
nb_angles = 100;
theta = 1:10:180;
%theta = 1:180/nb_angles:180;
p = 1001;

% Create the test problem.
[A,b,x] = paralleltomo(N,theta,p);

% load('100 angles phase maps V7')
% target_height = 500;
% sinogram = permute(phase_rotation_map(:,target_height,:),[1 3 2]);
% sinogram(1,:) = zeros(1,1001);
% sinogram(100,:) = zeros(1,1001);
% b = sinogram(:);

load('18 angles phase maps V7')
target_height = 500;
sinogram = permute(phase_rotation_map(:,target_height,:),[1 3 2]);
b = sinogram(:);
