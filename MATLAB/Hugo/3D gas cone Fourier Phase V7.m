...
theta_list = 1:180/nb_angles:180;
%theta_list = [0 15 30 45 90 135 180]
%theta_list = [33 55];
...
data = double(imrotate(I_screen,90,'crop'));
intref = double(imrotate(I0*(1+cos(delta_phi_tilt)),90));

fftim = fft2(data);
fftref = fft2(intref);

%Picking Fourier region
x2 = 480;
y2 = 428;
w = 12;
v = 137;
...
phase_rotation_map(nb_theta,:,:) = phase;
nb_theta = nb_theta+1
...
%sino = zeros(N,nb_angles);
sinogram = permute(phase_rotation_map(:,target_height,:),[1 3 2]);
imagesc(sinogram)
colorbar;
