...
for h = 1:N
    ...
    for i = 1:N
        for j = 1:N
            ...
            if d < ((r/(L/(N-1))/N))*h + r/(L/(N-1))  %Circular index profile at each height
                %index(i,j) = ((1-n)/(((r/(L/(N-1))/N))*h + r/(L/(N-1))))*d + n;
                %index(i,j) = n* exp(-(d/(2*sigma^2))^(-h*(8/N)+10) );
                index(i,j) = (n-1)* exp(-((d/(2*sigma^2))^(-h*(8/N)+10)));
            end
        end
    end

index((N+1)/2,(N+1)/2) = n-1;
delta_phi_gas(N+1-h,:) = (L/N)*(2*pi/lambda) .* sum(index,2)';
...
%%
imwrite(I0*(1+cos(delta_phi_tilt)), 'Ref_SupergaussianV5.jpg');     %Ref

imwrite(I_screen, 'Cone_SupergaussianV5.jpg');
